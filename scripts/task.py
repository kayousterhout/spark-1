  ...
  def __str__(self):
    if self.has_fetch:
      ...
      desc = (("Start time: %s, local read time: %s, " +
            ...
            "result ser: %s, finish: %s, shuffle bytes: %s, input bytes: %s, future task queue: %s, executor queue: %s") %
             (self.start_time, self.local_read_time,
              ...
              self.local_mb_read + self.remote_mb_read, self.input_mb, self.future_task_queue_time, self.executor_queue_delay)) 
    else:
