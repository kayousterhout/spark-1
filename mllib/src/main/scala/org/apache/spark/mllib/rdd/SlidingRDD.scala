...
import org.apache.spark.{TaskGoop, Partition}
...
class SlidingRDD[T: ClassTag](@transient val parent: RDD[T], val windowSize: Int)
  ...
  override def compute(split: Partition, goop: TaskGoop): Iterator[Seq[T]] = {
    val part = split.asInstanceOf[SlidingRDDPartition[T]]
    (firstParent[T].iterator(part.prev, goop) ++ part.tail)
      .sliding(windowSize)
      .withPartial(false)
  }
