...
class TaskSchedulerImplSuite extends FunSuite with LocalSparkContext with Logging {
  ...
  test("Scheduler assigns correct number of tasks based on task set's resource requirements") {
    ...
    def verifyNumberOfOffersAccepted(taskSet: TaskSet, numExpectedAcceptedOffers: Int): Unit = {
      taskScheduler.submitTasks(taskSet)
    ...
    val cpuTaskSet = createTaskSet(numTasks = 10, usesDisk = false, usesNetwork = false)
    verifyNumberOfOffersAccepted(cpuTaskSet, 1)

    // For a task set that uses the CPU and disk, 4 tasks should be assigned (one for each slot
    // except the network slot).
    val cpuAndDiskTaskSet = createTaskSet(numTasks = 10, usesDisk = true, usesNetwork = false)
    verifyNumberOfOffersAccepted(cpuAndDiskTaskSet, 4)

    // For a task set that uses the CPU and network, 2 tasks should be assigned.
    val cpuAndNetworkTaskSet = createTaskSet(numTasks = 10, usesDisk = false, usesNetwork = true)
    verifyNumberOfOffersAccepted(cpuAndNetworkTaskSet, 2)

    // For a task set that uses all three resources, all 5 slots should be used.
    val allResourcesTaskSet = createTaskSet(numTasks = 10, usesDisk = true, usesNetwork = true)
    verifyNumberOfOffersAccepted(allResourcesTaskSet, 5)
  }
}
