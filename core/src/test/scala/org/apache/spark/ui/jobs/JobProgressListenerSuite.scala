...
class JobProgressListenerSuite extends FunSuite with LocalSparkContext with Matchers {
  ...
  test("test executor id to summary") {
    ...
    var task = new ShuffleMapMacrotask(0, null, new Partition { override def index = 0 }, null)
    ...
    taskInfo.finishTime = 1
    task = new ShuffleMapMacrotask(0, null, new Partition { override def index = 0 }, null)
    listener.onTaskEnd(
      SparkListenerTaskEnd(task.stageId, 0, taskType, Success, taskInfo, taskMetrics))
  ...
  test("test task success vs failure counting for different task end reasons") {
    ...
    taskInfo.finishTime = 1
    val task = new ShuffleMapMacrotask(0, null, new Partition { override def index = 0 }, null)
    val taskType = Utils.getFormattedClassName(task)
