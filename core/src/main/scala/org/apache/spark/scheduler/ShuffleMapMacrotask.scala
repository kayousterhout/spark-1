 ...
 * if the configuration parameter spark.shuffle.writeToDisk is set to true, and it is stored
 * in memory otherwise.
 */
private[spark] class ShuffleMapMacrotask(
    ...
    private var writeShuffleDataToDisk: Boolean)
  ...
  override def getMonotasks(context: TaskContextImpl): Seq[Monotask] = {
    ...
    val maybeDiskWriteMonotask = if (writeShuffleDataToDisk) {
      ...
      val blockId = new MultipleShuffleBlocksId(dep.shuffleId, partition.index)
