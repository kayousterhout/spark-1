...
private[spark] class DiskScheduler(blockFileManager: BlockFileManager) extends Logging {
  ...
  /**
   * Returns a sequence of two item tuples, where the first item is a disk identifier and
   * the second item is the number of running and queued disk monotasks on that disk.
   */
