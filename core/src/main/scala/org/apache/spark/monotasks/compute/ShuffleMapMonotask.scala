...
private[spark] class ShuffleMapMonotask[T](
  ...
  override def cleanupIntermediateData(): Unit = {
    ...
    if (outputSingleBlock) {
      // Only need to cleanup the intermediate data if output is stored as a single block, in which
      ...
      SparkEnv.get.blockManager.removeBlockFromMemory(
        new MultipleShuffleBlocksId(dependency.shuffleId, partition.index),
