...
private[spark] class SampledRDD[T: ClassTag](
  ...
  override def compute(splitIn: Partition, goop: TaskGoop): Iterator[T] = {
    ...
    if (withReplacement) {
      ...
      firstParent[T].iterator(split.prev, goop).flatMap { element =>
      ...
      firstParent[T].iterator(split.prev, goop).filter(x => (rand.nextDouble <= frac))
    }
  }
}
