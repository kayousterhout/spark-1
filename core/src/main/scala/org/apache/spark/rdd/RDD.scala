...
abstract class RDD[T: ClassTag](
  ...
  private def getRdd(blockId: BlockId, context: TaskContext): Iterator[T] = {
    SparkEnv.get.blockManager.get(blockId).map{ blockResult =>
      logDebug(s"Found partition $blockId in memory.")
      val inputMetrics = blockResult.inputMetrics
      val existingMetrics = context.taskMetrics().getInputMetricsForReadMethod(
        inputMetrics.readMethod)
      existingMetrics.incBytesRead(inputMetrics.bytesRead)
      val block = blockResult.data.asInstanceOf[Iterator[T]]
      new InterruptibleIterator[T](context, block) {
        override def next(): T = {
      ...
      val errorMessage = s"RDD $blockId was not found in memory"
   ...
   * @param nextMonotask A monotask that performs some computation on this RDD. It will be
