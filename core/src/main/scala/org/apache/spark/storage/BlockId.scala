...
/**
 * Block ID describing a block that contains all of the all of the shuffle blocks output by a
 * particular map task.
 */
@DeveloperApi
case class MultipleShuffleBlocksId(shuffleId: Int, mapId: Int) extends BlockId {
  def name = "shuffle_" + shuffleId + "_" + mapId
}
...
object BlockId {
  ...
  val MULTIPLE_SHUFFLE = "shuffle_([0-9]+)_([0-9]+)".r
  ...
  def apply(id: String) = id match {
    ...
    case MULTIPLE_SHUFFLE(shuffleId, mapId) =>
      MultipleShuffleBlocksId(shuffleId.toInt, mapId.toInt)
