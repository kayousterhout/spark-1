...
object MemorySortJob {
  def main(args: Array[String]) {
    ...
    try {
      ...
      unsortedRdd.cache()
      unsortedRdd.mapPartitions { x =>
        // This is a hack to force the cached partitions to be evenly balanced across the different
        // executors. Without this sleep, there's too much variance in how long it takes to create
        // the cached blocks, and some executors end up creating much more data than others. That
        // is problematic later, because it means the map tasks (which are placed locally with the
        // input data) are imbalanced across machines.
        //Thread.sleep(2000)
        x
      }.count()
